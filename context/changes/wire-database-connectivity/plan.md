...
**Addendum (post-implementation, added during `/10x-impl-review`)**: The contract above is what was originally planned, but implementation actually used a different mechanism — a separate `TestcontainersDatasourceConfig` `@TestConfiguration` class with manually-managed `HikariDataSource` beans, gated by a `CI`/`!CI` Spring profile set in `build.gradle`'s `test` task, rather than `@Testcontainers`/`@Container`/`@ServiceConnection` directly on the test class. This was a necessary pivot, forced by a local VPN breaking Docker's default bridge networking (`@ServiceConnection`'s automatic container-type detection doesn't support the `GenericContainer`/host-network mode the fix requires) — full rationale is in `change.md`'s Notes. The substitute mechanism still satisfies this phase's verification intent: a real, ephemeral `postgres:17` container proves the datasource + Flyway + JPA stack works, in both CI (bridge network) and local (host network) modes.

**Addendum (post-implementation, added during `/10x-impl-review`)**: Also during this phase, at the user's explicit request (commit `3fb35bc`), Spring Boot's default Logback was swapped for Log4j2 (`spring-boot-starter-log4j2`, plus a minimal console-only `log4j2.xml`), and Lombok was added so `TestcontainersDatasourceConfig` could log which datasource (CI vs. local) is being initialized and its resolved JDBC URL, mirroring a reference logging pattern the user pointed to. Not part of the original Phase 2 contract, but not a silent addition either — recorded here for the plan's accuracy.
