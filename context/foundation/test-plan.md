...
|---|---|---|---|---|---|
| #1 | A request to any authenticated route after logout is rejected; a pre-logout session ID cannot be replayed to regain access | "Redirect to /login on logout" proves the response, not that the server-side session was invalidated | The actual session-management/logout-handler config; whether the existing logout test only checks the redirect | Integration test (MockMvc + Spring Security test support) | Asserting only the redirect status/location without a follow-up request proving unauthenticated state (already caught once — don't regress the lesson) |
...
| #5 | Mail-send failure paths never surface the raw Resend API key, SMTP credential, or a full stack trace in logs or client-visible error output | "We already catch `MailException` and don't 500" fixed availability — it says nothing about whether the log statement itself is safe; some JavaMail exceptions can echo the SMTP session transcript, including AUTH | Exactly what the `catch (MailException)` block logs today (message-only vs. full exception vs. stack trace); whether the log pattern could ever surface a credential from a nested cause | Unit test asserting the log/response never contains the configured credential value, using a stubbed failing mail send | Assuming the earlier availability fix also closed the confidentiality gap — they are different properties of the same catch block |
...
|---|---|---|---|---|---|---|
| 1 | Auth boundary & abuse-resistance coverage | Prove session integrity after logout/failed login, make the registration-throttle gap and log/error credential-leakage risk observable | #1, #4, #5 | unit + integration | change opened | `context/changes/testing-auth-boundary-abuse-resistance/` |
